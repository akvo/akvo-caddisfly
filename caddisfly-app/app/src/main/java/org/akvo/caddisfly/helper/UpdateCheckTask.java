...
public class UpdateCheckTask extends AsyncTask<Void, Void, Void> {
        ...
        if (NetworkUtil.checkInternetConnection(mContext, !mSilent)) {
            ...
            if (!mSilent) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    progressDialog = new ProgressDialog(mContext, android.R.style.Theme_Material_Light_Dialog_Alert);
                } else {
    ...
    protected Void doInBackground(Void... voids) {
        ...
        PreferencesUtil.setLong(mContext, R.string.lastUpdateCheckKey,
    ...
    private void alertUpdateNotFound() {
        AlertUtil.showMessage(mContext, R.string.noUpdate, R.string.updatedAlready);
    }
    ...
    private void alertUpdateAvailable(final String url, final String checksum, final String version) {
        AlertUtil.askQuestion(mContext, R.string.updateAvailable, R.string.updateRequest,
